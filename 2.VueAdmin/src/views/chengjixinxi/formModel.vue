				...
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="课程名称" prop="kechengmingcheng">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.kechengmingcheng?true:false"
								v-model="form.kechengmingcheng" 
								placeholder="请选择课程名称"
								>
								<el-option v-for="(item,index) in kechengmingchengLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="24">
						<el-form-item label="成绩" prop="chengji">
							<el-input class="list_inp" v-model.number="form.chengji" placeholder="成绩"
								 type="text" 								:readonly="!isAdd||disabledForm.chengji?true:false" />
						</el-form-item>
					</el-col>
...
<script setup>
	...
	//课程名称列表
	const kechengmingchengLists = ref([])
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.faburiqi = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='chengji'){
					form.value.chengji = row[x];
					disabledForm.value.chengji = true;
					continue;
				}
				if(x=='jiaoshigonghao'){
		...
		context?.$http({
			...
			kechengmingchengLists.value = res.data.data
		})
	}
	//初始化
	//声明父级调用

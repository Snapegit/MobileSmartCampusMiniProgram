		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="上课时间" prop="shangkeshijian">
							<el-date-picker
								class="list_date"
								v-model="form.shangkeshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.shangkeshijian?true:false"
								placeholder="请选择上课时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="课程状态" prop="kechengzhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.kechengzhuangtai?true:false"
								v-model="form.kechengzhuangtai" 
								placeholder="请选择课程状态"
								>
								<el-option v-for="(item,index) in kechengzhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	//课程状态列表
	const kechengzhuangtaiLists = ref([])
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='shangkeshijian'){
					form.value.shangkeshijian = row[x];
					disabledForm.value.shangkeshijian = true;
					continue;
				}
				if(x=='jiaoshigonghao'){
				...
				if(x=='kechengzhuangtai'){
					form.value.kechengzhuangtai = row[x];
					disabledForm.value.kechengzhuangtai = true;
					continue;
				}
			}
			if(row){
		...
		kechengzhuangtaiLists.value = "已开始,未开始".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
